...
export function marketTotals(s,m){
  const sales=s.sales.filter(x=>!x.void&&s.stocks.find(st=>st.id===x.stockId)?.marketId===m.id);
...
export function deliveryTotals(s,r){
  const map=new Map();
  const add=(id,name,kind,qty)=>{if(!map.has(id))map.set(id,{id,name,personal:0,onsite:0,event:0,external:0});map.get(id)[kind]+=qty;};
  for(const p of r.products)for(const o of Object.values(r.orders))add(p.id,p.name,'personal',o.quantities[p.id]||0);
  for(const st of s.stocks.filter(st=>st.roundId===r.id&&!st.sourceStockId&&!st.eventId))add(st.productId||st.id,st.name,st.channel==='external'?'external':'onsite',st.qty);
  for(const e of s.events.filter(e=>e.roundId===r.id))for(const l of e.lines)add(l.productId||l.id,l.name,'event',l.qty);
  return [...map.values()].map(x=>({...x,qty:x.personal+x.onsite+x.event+x.external})).filter(x=>x.qty>0);
}
