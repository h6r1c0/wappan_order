...
export function LineImport({round,onClose}){
 ...
 return <Modal title="LINE注文を貼り付け" onClose={onClose}><p>一人分の注文を貼り付け、候補を確認してから反映します。LINEへの接続や自動送信は行いません。</p><Field label="注文文章"><textarea rows="6" placeholder={'近藤\nカリカリ1\n黒糖1'} value={text} onChange={e=>{setText(e.target.value);setParsed(null);}} /></Field><Button onClick={()=>{const p=parseLineOrder(text,state,round);if(p.suggestedBuyerName)p.unread=p.unread.map(x=>({...x,ignored:true}));setParsed(p);setBuyer(p.buyerId);}}>注文候補を読み取る</Button>{parsed&&<>
 <h3>① 購入者を確認</h3>{parsed.buyers.length>1&&<p className="notice">複数の購入者名があります。一人分ずつに分けてください。</p>}{parsed.suggestedBuyerName&&<p className="notice">未登録の購入者名候補：<strong>{parsed.suggestedBuyerName}</strong></p>}<BuyerPicker value={buyer} onChange={setBuyer} includeTest={round.test} suggestedName={buyer?"":parsed.suggestedBuyerName}/>
 ...
 {!parsed.suggestedBuyerName&&parsed.unread.map((p,i)=><div className="notice" key={i}>{p.destinationCandidate?.type==='external'?<>登録済み購入者に一致しません：<strong>{p.raw}</strong><br/>外部販売・団体名の候補です。LINE注文には自動登録しません。</>:<>読み取れない行：{p.raw}</>}<Check label="注文ではない行として除外する" checked={p.ignored} onChange={v=>setParsed({...parsed,unread:parsed.unread.map((x,j)=>i===j?{...x,ignored:v}:x)})}/></div>)}
