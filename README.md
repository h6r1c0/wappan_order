...
2. 「注文」で納品日を作り、購入者→数量→保存。集金額と発注数を確認する。
3. 欠品は了承した購入者の数量を修正。「納品・精算」で納品書の仕入税込総額を入力する。
4. 園内販売は販売先または即時入金を登録。行事の余剰は「行事」から園内販売へ振り替える。
5. 「集計」で年度利益、個人請求、過去実績、利益調整を確認する。

[9月11日分の照合手順](docs/ACCEPTANCE.md) / [会計・保存設計](docs/DESIGN.md)
...
- SupabaseのDB migration: `supabase/migrations/（番号順の2ファイル）`
...
購入者向け公開フォーム、LINE連携、納品書OCR、通知、商品ごとの画像管理、マーケティング、複雑な在庫・行事管理は含みません。
