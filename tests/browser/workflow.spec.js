...
test("スマホ: Excel→固定注文→欠品→精算→おやつ余剰振替→後日請求・即時入金→年度・再読込", async ({
  ...
  await page.getByLabel("くるみパン 数量", { exact: true }).selectOption("other");
  await page.getByLabel("くるみパン 数量 その他", { exact: true }).fill("20");
  await page.getByLabel("こしあんぱん 数量", { exact: true }).selectOption("other");
  await page.getByLabel("こしあんぱん 数量 その他", { exact: true }).fill("20");
  ...
  await dialog.getByLabel("くるみパン おやつ使用数", { exact: true }).selectOption("other");
  await dialog.getByLabel("くるみパン おやつ使用数 その他", { exact: true }).fill("18");
  await page.getByRole("button", { name: "おやつ用の注文・使用数を保存" }).click();
  await expect(
    page.getByText("2,484円", { exact: true }).first(),
  ).toBeVisible();
  await page.locator('.event-line').filter({hasText:'くるみパン'}).getByRole("button", { name: "余剰を販売用へ振替" }).click();
